...
def display_panel():
    """Busca e exibe os eventos do serviço de ingestão de dados."""
    while True:
        try:
            # A URL do serviço Flask, acessível dentro da mesma rede Docker
            response = requests.get('http://data-ingestion-service:3001/api/eventos')
            response.raise_for_status()  # Lança uma exceção para respostas com erro
            eventos = response.json()

            clear_screen()
            print("--- Painel de Eventos Recentes (Data Ingestion Service) ---")
            print("-" * 60)
            ...
            clear_screen()
            print(f"Não foi possível conectar ao serviço de ingestão de dados: {e}")
